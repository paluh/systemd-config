...
data Match = Match
  { -- Checks whether the system is running on a specific architecture. See
    ...
    -- currently bound to the device, matchas exposed by the udev property "DRIVER"
    -- of its parent device, matchor if that is not set the driver as exposed by
    -- "ethtool -i" of the device itself.
  , matchDriver :: Maybe Text
    -- Matches against the hostname or machine ID of the host. See "ConditionHost="
...
data Network = Network
  { -- A description of the device. This is only used for presentation purposes.
    ...
    -- The format of the address must be as described in inet_pton(3). This is a
    -- short-hand for an [Address] section only containing an Address key (see
    -- below). This option may be specified more than once.
    --
    -- If the specified address is 0.0.0.0 (for IPv4) or [::] (for IPv6), a new
    -- address range of the requested size is automatically allocated from a
    -- system-wide pool of unused ranges. The allocated range is checked against all
    -- current network interfaces and all known network configuration files to avoid
    -- address range conflicts. The default system-wide pool consists of
    -- 192.168.0.0/16, 172.16.0.0/12 and 10.0.0.0/8 for IPv4, and fc00::/7 for IPv6.
    -- This functionality is useful to manage a large number of dynamically created
  ...
  , networkDNS :: [IPv4]
    -- Enables DHCPv4 and/or DHCPv6 client support. Accepts "yes", "no", "ipv4", or
    -- "ipv6".
    --
    ...
    -- not used for name resolution. See option UseDomains= below.
    --
    -- See the "[DHCP]" section below for further configuration options for the DHCP
    -- client support.
  , networkDHCP :: Maybe NetworkDHCP
    -- The name of the bridge to add the link to.
  , networkBridge :: Maybe Text
    -- The name of a MACVLAN to create on the link. This option may be specified
    -- more than once.
